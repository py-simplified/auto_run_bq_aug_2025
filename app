...
def classify_derivation_logic(derivation_logic: str) -> str:
    """
    Classify derivation logic as either simple direct mapping or complex.
    
    Args:
        derivation_logic: The derivation logic string to classify
        
    Returns:
        'simple' for strings with exactly one dot (table.column), 'complex' for anything else
    """
    if not derivation_logic or str(derivation_logic).strip().lower() in ['nan', 'none', '']:
        return 'complex'  # Treat empty/null as complex to be safe
    
    # Clean the derivation logic
    cleaned = str(derivation_logic).strip()
    
    # Simple rule: exactly one dot means simple mapping
        ...
        if uploaded_file.type in ['text/csv', 'application/vnd.ms-excel', 
            ...
            try:
                if uploaded_file.name.endswith('.csv'):
                    # Read headers and data separately to handle potential parsing issues
                    headers = pd.read_csv(uploaded_file, nrows=1, header=None).iloc[0].tolist()
                    df = pd.read_csv(uploaded_file, header=None, skiprows=1)
                    df.columns = [str(h).strip() for h in headers]
                else:
                    # For Excel, read the first row as headers
                    headers = pd.read_excel(uploaded_file, nrows=1, header=None).iloc[0].tolist()
                ...
                # Classify scenarios into two groups and show grouped preview
                simple_scenarios = []
                complex_scenarios = []
                ...
                # Find the derivation logic column
                derivation_col_options = ['derivation_logic', 'Derivation_Logic', 'derivation', 'logic', 'formula', 'expression']
                derivation_col = None
                
                for col_option in derivation_col_options:
                    if col_option in df.columns:
                        derivation_col = col_option
                        break
                
                if not derivation_col:
                    # Default to first column that might contain derivation data
                    for col in df.columns:
                        if any(keyword in col.lower() for keyword in ['derivation', 'logic', 'formula']):
                            derivation_col = col
                            break
                
                if not derivation_col:
                    st.error("Could not find derivation logic column in the uploaded file.")
                    return
                
                # Classify scenarios using the correct column
                for _, row in df.iterrows():
                    derivation_logic = str(row.get(derivation_col, ''))
                    classification = classify_derivation_logic(derivation_logic)
                    
                    if classification == 'simple':
                        simple_scenarios.append(row)
                    else:
                        complex_scenarios.append(row)
                
                # Display grouped file preview
                st.write("**File Preview (Grouped by Derivation Complexity):**")
                
                # Group 1 - Simple Mappings
                st.write(f"**🟢 Group 1: Simple Mappings ({len(simple_scenarios)} scenarios)**")
                st.write("*Direct table.column mappings - will be SKIPPED*")
                if simple_scenarios:
                    simple_df = pd.DataFrame(simple_scenarios)
                    st.dataframe(simple_df.head(), use_container_width=True)
                else:
                    st.info("No simple mappings found")
                
                st.divider()
                
                # Group 2 - Complex Derivations  
                st.write(f"**🔴 Group 2: Complex Derivations ({len(complex_scenarios)} scenarios)**")
                st.write("*Functions, expressions, calculations - will be PROCESSED*")
                if complex_scenarios:
                    complex_df = pd.DataFrame(complex_scenarios)
                    st.dataframe(complex_df.head(), use_container_width=True)
                else:
                    st.info("No complex derivations found")
                ...
                if run_clicked:
                    ...
                    with log:
                        ...
                        tab_conn, tab_scen, tab_files = st.tabs(["Connectivity", "Scenarios", "Files"])
                    ...
                    # Step 3: Enhanced Validation Framework Execution
                            with log:
                                with tab_scen:
                                    st.write("**Step 2: Enhanced Validation Framework Execution**")
                            ...
                            # Step 3: Group scenarios by derivation logic complexity
                            with log:
                                with tab_scen:
                                    st.write(f"🔄 Step 3: Analyzing and grouping {len(df)} validation scenarios...")
                                    st.info("🚀 Grouping scenarios: Simple direct mappings vs Complex derivations")
                            
                            # Group scenarios based on derivation logic
                            simple_scenarios = []
                            complex_scenarios = []
                            ...
                            for idx, row in df.iterrows():
                                ...
                                if classification == 'simple':
                                    simple_scenarios.append(scenario_info)
                                else:
                                    complex_scenarios.append(scenario_info)
                            
                            # Display grouping results
                            with log:
                                with tab_scen:
                                    st.write("**📊 Scenario Classification Results:**")
                                    st.write(f"• **Group 1 - Simple Direct Mappings**: {len(simple_scenarios)} scenarios")
                                    st.write(f"• **Group 2 - Complex Derivations**: {len(complex_scenarios)} scenarios")
                                    
                                    # Show previews of each group
                                    if simple_scenarios:
                                        st.write("**🔍 Group 1 Preview (Simple Mappings):**")
                                        simple_preview = []
                                        for i, scenario in enumerate(simple_scenarios[:5]):  # Show first 5
                                            simple_preview.append({
                                                'Scenario ID': scenario['scenario_id'],
                                                'Derivation Logic': scenario['derivation_logic']
                                            })
                                        st.dataframe(pd.DataFrame(simple_preview), use_container_width=True)
                                        if len(simple_scenarios) > 5:
                                            st.write(f"... and {len(simple_scenarios) - 5} more simple scenarios")
                                    
                                    if complex_scenarios:
                                        st.write("**🔍 Group 2 Preview (Complex Derivations):**")
                                        complex_preview = []
                                        for i, scenario in enumerate(complex_scenarios[:5]):  # Show first 5
                                            complex_preview.append({
                                                'Scenario ID': scenario['scenario_id'],
                                                'Derivation Logic': scenario['derivation_logic']
                                            })
                                        st.dataframe(pd.DataFrame(complex_preview), use_container_width=True)
                                        if len(complex_scenarios) > 5:
                                            st.write(f"... and {len(complex_scenarios) - 5} more complex scenarios")
                            
                            # Initialize result tracking lists with default values, matching the DataFrame length
                            num_scenarios = len(df)
                            processing_methods = ["Not Processed"] * num_scenarios
                            total_passed = [0] * num_scenarios
                            total_failed = [0] * num_scenarios
                            overall_status = ["Not Processed"] * num_scenarios
                            failure_files_created = ["Not Processed"] * num_scenarios
                            sql_queries = ["Not Processed"] * num_scenarios
                            
                            # Create a status container inside the expander
                            status_text = log.empty()
                            
                            # Process Group 1 (Simple scenarios) - Update status for skipped scenarios
                            if simple_scenarios:
                                with log:
                                    with tab_scen:
                                        st.write(f"⏭️ **Processing Group 1**: Skipping {len(simple_scenarios)} simple direct mapping scenarios")
                                
                                for scenario_info in simple_scenarios:
                                    idx = scenario_info['idx']
                                    scenario_id = scenario_info['scenario_id']
                                    derivation_logic = scenario_info['derivation_logic']
                                    
                                    # Update results at the specific index
                                    processing_methods[idx] = "SKIPPED - Simple Direct Mapping"
                                    overall_status[idx] = 'SKIPPED_SIMPLE_MAPPING'
                                    failure_files_created[idx] = "Not processed - Simple direct mapping"
                                    sql_queries[idx] = f"-- Skipped: Simple mapping {derivation_logic}"
                                    
                                    with log:
                                        with tab_scen:
                                            st.write(f"⏭️ Scenario {scenario_id}: SKIPPED_SIMPLE_MAPPING ({derivation_logic})")
                            
                            # Process Group 2 (Complex scenarios) - Use existing validation logic
                            if complex_scenarios:
                                with log:
                                    with tab_scen:
                                        st.write(f"🚀 **Processing Group 2**: Executing validation for {len(complex_scenarios)} complex derivation scenarios")
                                
                                for i, scenario_info in enumerate(complex_scenarios):
                                    # Update progress based on complex scenarios
                                    progress = (i + 1) / len(complex_scenarios) if complex_scenarios else 1
                                    progress_bar.progress(progress)
                                    
                                    row = scenario_info['row']
                                    idx = scenario_info['idx']
                                    scenario_id = scenario_info['scenario_id']
                                    ...
                                    with log:
                                        with tab_scen:
                                            status_text.write(f"🔍 Processing complex scenario {i+1}/{len(complex_scenarios)}: {scenario_id} - {scenario_name}")
                                    ...
                                    try:
                                        ...
                                        with log:
                                            with tab_scen:
                                                ...
                                                with st.container():
                                                    st.caption(f"Scenario Details for {scenario_id}")
                                                derivation_logic = row.get('Derivation_Logic', '')
                                                source_cols, target_cols, ref_cols = extract_required_columns_by_table(row)
                                        ...
                                        if success:
                                            ...
                                            total_passed[idx] = pass_count
                                            total_failed[idx] = fail_count
                                            overall_status[idx] = 'PASS' if fail_count == 0 else 'FAIL'
                                            processing_methods[idx] = "Enhanced Framework - SQL Pushdown"
                                            sql_queries[idx] = executed_sql if executed_sql else "No SQL available"
                                            ...
                                            if failure_file_path:
                                                if failure_file_path.startswith("SKIPPED"):
                                                    ...
                                                    failure_files_created[idx] = failure_file_path
                                                elif failure_file_path == "No failures":
                                                    failure_files_created[idx] = "No failures"
                                                    ...
                                                    failure_files_created[idx] = relative_path
                                            else:
                                                failure_files_created[idx] = "No failures"
                                        else:
                                            with log:
                                            ...
                                            processing_methods[idx] = f"EXECUTION_ERROR - {message}"
                                            overall_status[idx] = 'EXECUTION_ERROR'
                                            failure_files_created[idx] = f"EXECUTION_ERROR - {message}"
                                            sql_queries[idx] = executed_sql if executed_sql else "-- SQL execution failed --"
                                        ...
                                        if "do not uniquely identify rows" in str(ve):
                                            ...
                                            processing_methods[idx] = f"UNIQUENESS_ERROR - {str(ve)}"
                                            overall_status[idx] = 'UNIQUENESS_ERROR'
                                            failure_files_created[idx] = f"Uniqueness Error: {str(ve)}"
                                            sql_queries[idx] = "-- Uniqueness validation failed --"
                                        else:
                                            with log:
                                                with tab_scen:
                                                    st.error(f"❌ Validation error for scenario {scenario_id}: {str(ve)}")
                                            processing_methods[idx] = f"VALIDATION_ERROR - {str(ve)}"
                                            overall_status[idx] = 'VALIDATION_ERROR'
                                            failure_files_created[idx] = f"Validation Error: {str(ve)}"
                                            sql_queries[idx] = "-- Validation error occurred --"
                                    except Exception as e:
                                        with log:
                                            with tab_scen:
                                                st.error(f"❌ Error processing scenario {scenario_id}: {str(e)}")
                                        processing_methods[idx] = f"PROCESSING_ERROR - {str(e)}"
                                        overall_status[idx] = 'ERROR'
                                        failure_files_created[idx] = f"Error: {str(e)}"
                                        sql_queries[idx] = "-- Processing error occurred --"
